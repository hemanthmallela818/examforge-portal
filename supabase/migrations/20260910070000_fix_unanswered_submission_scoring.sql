-- Missing statuses and JSON null answers must never incur incorrect-answer marks.
...
BEGIN
  ...
  FOR answer_entry IN SELECT key, value FROM jsonb_each(answers_obj) LOOP
    ...
    is_attempted := COALESCE(response_data->>'status' IN ('ANSWERED', 'ANSWERED_MARKED')
      AND NULLIF(btrim(response_data->>'selected_option'), '') IS NOT NULL, false);
    is_correct := false;
