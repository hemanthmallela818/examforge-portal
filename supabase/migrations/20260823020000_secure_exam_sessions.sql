-- Secure server-owned exam sessions and grading.
-- Requires the answer-safe cbt_exams view, cbt_exams_raw, and
-- cbt_exam_answers objects already present in the staging/production schema.
...
CREATE UNIQUE INDEX IF NOT EXISTS student_results_student_exam_key
  ON public.student_results (student_id, exam_id);
...
BEGIN
  ...
  END IF;
  IF exam_row.class IS DISTINCT FROM student_row.class
     OR exam_row.section IS DISTINCT FROM student_row.section THEN
  ...
  IF FOUND THEN
    IF session_row.started_at IS NULL
       OR clock_timestamp() > session_row.started_at + make_interval(secs => duration_seconds + 180) THEN
      PERFORM public.terminate_exam(exam_id_param);
      RAISE EXCEPTION 'This exam session has expired';
    END IF;
    RETURN jsonb_build_object(
      'time_left', session_row.time_left,
  ...
  INSERT INTO public.active_sessions (
    ...
    time_left, started_at, updated_at
  ) VALUES (
    session_id, student_row.student_id, exam_id_param::text, responses_param,
    exam_data_param, duration_seconds, now(), now()
  );

  RETURN jsonb_build_object(
    'time_left', duration_seconds,
    'jumbled_exam_data', exam_data_param,
...
BEGIN
  ...
  IF FOUND THEN
    RETURN jsonb_build_object(
      'totalScore', existing_result.total_score,
      'maxScore', existing_result.max_score,
      'correct', existing_result.correct,
      'incorrect', existing_result.incorrect,
      'unattempted', existing_result.unattempted,
      'subjectScores', existing_result.subject_scores
    );
  END IF;
  ...
  FOR answer_entry IN SELECT key, value FROM jsonb_each(answers_obj) LOOP
    ...
    subject_scores := jsonb_set(
      subject_scores,
      ARRAY[answer_data->>'subject'],
      COALESCE(subject_scores->(answer_data->>'subject'), '0'::jsonb),
      true
    );

    is_attempted := response_data->>'status' IN ('ANSWERED', 'ANSWERED_MARKED')
      AND response_data->'selected_option' IS NOT NULL
      AND response_data->>'selected_option' <> '';
    is_correct := false;
  ...
  INSERT INTO public.student_results (
    ...
    exam_id_param::text, student_row.student_id, student_row.name,
    total_score, total_questions * marks_correct, correct_count,
    incorrect_count, unattempted_count, subject_scores, now()
  );
  ...
  RETURN jsonb_build_object(
    ...
    'maxScore', total_questions * marks_correct,
