-- Candidate writes must go through server-owned exam functions. RLS alone was
-- not sufficient: a candidate could previously insert their own arbitrary score
-- or delete their own session and obtain a fresh deadline.
...
REVOKE INSERT, UPDATE ON public.student_results FROM anon, authenticated;
...
REVOKE INSERT, UPDATE, DELETE ON public.active_sessions FROM anon, authenticated;
GRANT INSERT, DELETE ON public.active_sessions TO authenticated;
GRANT UPDATE (user_responses, updated_at) ON public.active_sessions TO authenticated;

-- Resume time is calculated from the immutable server timestamp. It does not
-- trust a browser-supplied time_left value.
...
-- Ending an exam blocks new sessions, but candidates who already have a valid
-- session must still be able to submit during their server-owned deadline.
